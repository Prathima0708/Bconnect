...
const Cart = (props) => {
    ...
    const fetchUserAddress = async () => {
        try {
            ...
            const res = await axios.post(`${getUserAddress}`, request_body, { headers });
    ...
    const getUserCart = async () => {
        ...
        const headers = { 'Content-Type': 'application/json; charset=utf-8' };
        try {
            const res = await axios.post(`${userCart_URL}`, request_model, { headers });
            const productIdArray = res.data.flatMap((item) => item.productQuantity && item.productQuantity.productId ? [item.productQuantity.productId] : []);
    ...
    const calculateTotal = () => {
        ...
        userCart.forEach((item) => {
            sum += item.productQuantity.quantity * (item.productQuantity.product ? item.productQuantity.product.price : 0);
        });
        setTotal(sum);
    };
    ...
    const placeOrder = async () => {
        ...
        try {
            const headers = { 'Content-Type': 'application/json; charset=utf-8' };
            const res = await axios.post(`${placeOrder_URL}`, request_body, { headers });
    ...
    const deleteCartItem = async (id) => {
        try {
            console.log('Deleting item:', id);
            const request_model = { id: id };
            const headers = { 'Content-Type': 'application/json; charset=utf-8' };
            const res = await axios.post(`${deleteCartItem_URL}`, request_model, { headers });
    ...
    const clearCart = async () => {
        ...
        try {
            const headers = { 'Content-Type': 'application/json; charset=utf-8' };
            const res = await axios.post(`${deleteUserCart_URL}`, request_body, { headers });
